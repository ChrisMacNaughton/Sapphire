module Sapphire
  ...
  class ASTNode
    attr_writer :line_number, :column_number
    ...
    def line_number
      defined?(@line_number) ? @line_number : 0
    end

    def column_number
      defined?(@column_number) ? @column_number : 0
    end
  end
  ...
  #
  class Call < ASTNode
    attr_accessor :obj, :name, :args, :block, :name_column_number
